        ...
        if await is_maintenance() is False:
            if message.from_user.id not in SUDOERS:
                return await message.reply_text(
                    text=f"{app.mention} is under maintainance, visit <a href={SUPPORT_CHAT}>Support Chat</a> for knowing the reason.",
...
Refresh admincache via : /reload

» {upvote} votes needed for promoting this action."""
        ...
        if await is_maintenance() is False:
            if CallbackQuery.from_user.id not in SUDOERS:
                return await CallbackQuery.answer(
                    f"{app.mention} is under maintainance, visit Support Chat for knowing the reason.",
                    show_alert=True,
                )
        try:
